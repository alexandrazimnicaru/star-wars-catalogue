...
const getPeople = async (apiResponse) => {
  try {
...
export const getPeopleWithCount = async (page = 1) => {
  try {
...
export const searchPeopleWithCount = async (search = '', page = 1) => {
  try {
    const response = await fetch(`${BASE_URL}/people/?search=${search}&page=${page}`);
    const people = await getPeople(response);
    return people;
  } catch(err) {
    publish(SHOW_ERROR, GET_PEOPLE_ERROR);
